...
export const DashboardScreen: React.FC = () => {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/services/app/Coach/GetAll`, { headers: getRequestHeaders() });
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/services/app/CoachMember/GetAll`, { headers: getRequestHeaders() });
        // Token expired or invalid — clear auth and redirect to login
        if (res.status === 401) {
          ['_auth_token', '_bearer_token', '_refresh_token'].forEach(k => localStorage.removeItem(k));
          if (window.location.pathname !== '/login') window.location.href = '/login';
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/services/app/SessionPackage/GetAll`, { headers: getRequestHeaders() });
        // Token expired or invalid — clear auth and redirect to login
        if (res.status === 401) {
          ['_auth_token', '_bearer_token', '_refresh_token'].forEach(k => localStorage.removeItem(k));
          if (window.location.pathname !== '/login') window.location.href = '/login';
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: 16 }}>
        ...
        <div style={{ gridColumn: 'span 3' }}>
          ...
          <TkCard>
            <div slot="content" style={{ padding: 20 }}>
              <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>Total Hoca-Üye Ataması</div>
        ...
        <div style={{ gridColumn: 'span 3' }}>
          ...
          <TkCard>
            <div slot="content" style={{ padding: 20 }}>
              <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>Total Seans Paketi</div>
