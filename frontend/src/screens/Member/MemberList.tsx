...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'sessionBalance', header: 'Session Balance', sortable: true },
  { field: 'status', header: 'Status', sortable: true, filterType: 'radio', filterOptions: [{ label: 'Active', value: '0' }, { label: 'Frozen', value: '1' }, { label: 'Passive', value: '2' }], html: (row: Record<string, unknown>) => { const m: Record<string, string> = {'0': 'Active', '1': 'Frozen', '2': 'Passive'}; return m[String(row.status ?? '')] ?? String(row.status ?? '\u2014'); } },
  { field: 'frozenUntil', header: 'Frozen Until', sortable: true, filterType: 'datepicker', html: (row: Record<string, unknown>) => row.frozenUntil ? new Date(String(row.frozenUntil)).toLocaleDateString() : '—' },
