...
import { SessionPackageCreate } from './SessionPackageCreate';
import { SessionPackageEdit } from './SessionPackageEdit';
...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'packageName', header: 'Package Name', sortable: true, searchable: true, filterType: 'text' },
  { field: 'sessionCount', header: 'Session Count', sortable: true },
...
export const SessionPackageList: React.FC = () => {
  const list = useListQuery<SessionPackageRecord>({ resource: 'SessionPackage' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<SessionPackageRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<SessionPackageRecord[]>([]);

  const del = useDeleteMutation('SessionPackage');
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as SessionPackageRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create SessionPackage" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <SessionPackageCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <SessionPackageEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
