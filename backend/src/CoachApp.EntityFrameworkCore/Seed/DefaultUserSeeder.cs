...
namespace CoachApp.EntityFrameworkCore.Seed
{
    /// <summary>
    ...
    public class DefaultUserSeeder
    {
        ...
        public void Seed()
        {
            ...
            try
            {
                ...
                Console.WriteLine("[Seed] Step 5: Custom role 'Coach'");
                var customRole_0 = _roleRepo.GetAll().FirstOrDefault(r => r.Name == "Coach");
                if (customRole_0 == null)
                {
                    customRole_0 = new AppRole { Name = "Coach", DisplayName = "Hoca", Description = "Spor salonu hocası — tüm üyeleri görüntüler, kendi derslerini ve katılım kayıtlarını oluşturur, seans ve ödeme girişi yapar", IsSystem = false, IsActive = true };
                    customRole_0.Id = _roleRepo.InsertAndGetId(customRole_0);
                    Console.WriteLine($"[Seed] Created custom role 'Coach' (id={customRole_0.Id})");
                }
                var customPerms_0 = new[] { "Member.Read", "CoachMember.Read", "CoachMember.Create", "Lesson.Read", "Lesson.Create", "Lesson.Update", "Lesson.ChangeStatus", "Attendance.Read", "Attendance.Create", "Attendance.Update", "Payment.Create", "Payment.Read", "SessionPackage.Read" };
                ...
                {
                    if (!allPermSet_0.Contains(pn)) { Console.WriteLine($"[Seed] Skipping unknown permission '{pn}' for role 'Coach'"); continue; }
