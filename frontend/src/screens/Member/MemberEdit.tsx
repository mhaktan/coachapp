        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                ...
                <div>
                  <TkInput mode="number" label="Session Balance *" value={String(form.sessionBalance ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('sessionBalance', Number(v)))(e.detail)} />
                </div>
                <div>
                  <LookupSelect label="Status *" value={String(form.status ?? '')} onChange={(v) => setField('status', v ? Number(v) : null)} searchable={false} options={[{ label: 'Active', value: '0' }, { label: 'Frozen', value: '1' }, { label: 'Passive', value: '2' }]} />
                </div>
                <div>
                  <TkDatepicker label="Frozen Until" value={String(form.frozenUntil ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('frozenUntil', v))(e.detail)} />
                </div>
                <div>
