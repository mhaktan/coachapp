...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'sessionDeducted', header: 'Session Deducted', sortable: true, filterType: 'checkbox', filterOptions: [{ label: 'Yes', value: 'true' }, { label: 'No', value: 'false' }], html: (row: Record<string, unknown>) => row.sessionDeducted ? '<span style="color:#2e7d32;font-weight:600">Yes</span>' : '<span style="color:#999">No</span>' },
...
export const AttendanceList: React.FC = () => {
  const list = useListQuery<AttendanceRecord>({ resource: 'Attendance' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<AttendanceRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<AttendanceRecord[]>([]);

  const del = useDeleteMutation('Attendance');
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as AttendanceRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create Attendance" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <AttendanceCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <AttendanceEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
