...
import { CoachMemberCreate } from './CoachMemberCreate';
import { CoachMemberEdit } from './CoachMemberEdit';

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

type CoachMemberRecord = {
  id: string | number;
  assignedAt: string;
  isActive: boolean;
  memberId: string;
...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'assignedAt', header: 'Assigned At', sortable: true, filterType: 'datepicker', html: (row: Record<string, unknown>) => row.assignedAt ? new Date(String(row.assignedAt)).toLocaleDateString() : '—' },
  ...
  { field: 'coachId', header: 'Hoca', sortable: true },
];
...
export const CoachMemberList: React.FC = () => {
  const list = useListQuery<CoachMemberRecord>({ resource: 'CoachMember' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<CoachMemberRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<CoachMemberRecord[]>([]);

  const del = useDeleteMutation('CoachMember');
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as CoachMemberRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create CoachMember" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <CoachMemberCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <CoachMemberEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
