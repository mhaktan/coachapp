...
interface CoachMemberEditProps {
  record: CoachMemberRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const CoachMemberEdit: React.FC<CoachMemberEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<CoachMemberRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<CoachMemberRecord>) =>
      dataProvider.update('CoachMember', record!.id, values),
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit CoachMember</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
                  <TkDatepicker label="Assigned At *" value={String(form.assignedAt ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('assignedAt', v))(e.detail)} />
                </div>
                <div>
