...
export const CoachList: React.FC = () => {
  const list = useListQuery<CoachRecord>({ resource: 'Coach' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<CoachRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<CoachRecord[]>([]);

  const del = useDeleteMutation('Coach');
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as CoachRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create Coach" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <CoachCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <CoachEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
