...
interface CoachMemberCreateProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export const CoachMemberCreate: React.FC<CoachMemberCreateProps> = ({ open, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<CoachMemberRecord>>({});
  const setField = (name: string, value: unknown) => setForm((p) => ({ ...p, [name]: value }));

  const mutation = useMutation({
    mutationFn: (values: Partial<CoachMemberRecord>) => dataProvider.create('CoachMember', values),
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Create CoachMember</h2>
