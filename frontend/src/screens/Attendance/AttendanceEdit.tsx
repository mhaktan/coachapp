...
export const AttendanceEdit: React.FC<AttendanceEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<AttendanceRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<AttendanceRecord>) =>
      dataProvider.update('Attendance', record!.id, values),
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit Attendance</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                ...
                <div>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14 }}>
                    <input type="checkbox" checked={!!form.sessionDeducted} onChange={(e) => ((v) => setField('sessionDeducted', v))(e.target.checked)} style={{ width: 16, height: 16 }} />
                    Session Deducted *
                  </label>
                </div>
                <div>
